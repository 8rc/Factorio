...
class goto_modal(discord.ui.Modal, title="CadenAI"):
    def __init__(self, button):
        super().__init__()
        self.button = button
        self.page_num = discord.ui.TextInput(
            label="Paginator Menu",
...
class Paginator(discord.ui.View):
    ...
    def add_button(self, action, /, *, label="", emoji=None, row=None):
        defer = self.defer
        action = action.strip().lower()
        if action not in ["first", "prev", "previous", "back", "delete", "next", "last", "end", "page", "show", "goto", "lock"]:
            return
        elif action == "first":
            self.add_item(first_page(label, emoji, discord.ButtonStyle.blurple, row))
        elif action in ["back", "prev", "previous"]:
            self.add_item(prev_page(label, emoji, discord.ButtonStyle.blurple, row))
        elif action in ["page", "show"]:
            button = show_page("yes", emoji, discord.ButtonStyle.blurple, row)
            self.page_button = button
            self.add_item(button)
            self.update_view()
        elif action == "goto":
            button = goto_page(None, emoji, discord.ButtonStyle.blurple, row)
            self.page_button = button
            self.add_item(button)
            self.update_view()
        elif action == "next":
            self.add_item(next_page(label, emoji, discord.ButtonStyle.blurple, row))
        elif action == "last":
            self.add_item(last_page(label, emoji, discord.ButtonStyle.blurple, row))
        elif action == "end":
            self.add_item(end_page(label, emoji, discord.ButtonStyle.blurple, row))
        elif action == "delete":
            self.add_item(delete_page(label, emoji, discord.ButtonStyle.blurple, row))
        elif action == "lock":
            self.add_item(lock_page(label, emoji, discord.ButtonStyle.blurple, row))